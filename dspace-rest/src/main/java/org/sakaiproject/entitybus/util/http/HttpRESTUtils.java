/**
 * $Id: HttpRESTUtils.java 94 2009-10-31 13:33:47Z azeckoski $
 * $URL: http://entitybus.googlecode.com/svn/tags/entitybus-1.0.8/utils/src/main/java/org/sakaiproject/entitybus/util/http/HttpRESTUtils.java $
...
import org.sakaiproject.entitybus.util.http.HttpAuth.AuthType;
...
public class HttpRESTUtils {
    ...
    public static HttpResponse fireRequest(HttpClientWrapper httpClientWrapper, String URL, Method method, Map<String, String> params, Map<String, String> headers, Object data, boolean guaranteeSSL) {
        ...
        if (method.equals(Method.GET)) {
            ...
            if (data != null) {
                System.out.println("WARN: data cannot be passed in GET requests, data will be ignored (HttpRESTUtils#fireRequest)");
            }
                ...
                for (Entry<String, String> entry : params.entrySet()) {
                    if (entry.getKey() == null
                        ...
                        System.out.println("WARN: null value supplied for param name ("+entry.getKey()+") or value ("
                                +entry.getValue()+") (HttpRESTUtils#fireRequest)");
                    }
            ...
            // warn about data being set
            if (data != null) {
                System.out.println("WARN: data cannot be passed in DELETE requests, data will be ignored (HttpRESTUtils#fireRequest)");
            }
            httpMethod = dm;
        } else {
    ...
    /**
     * Takes auth data and encodes it into a valid value to place into an "Authorization" header
     * to add basic auth to a request
     * @param httpAuth username and password data
     * @return the encoded string to place into the Authorization header
     */
    public static String makeAuthorizationHeaaderValue(HttpAuth httpAuth) {
        String encoded = null;
        if (httpAuth != null) {
            String toEncode = httpAuth.getLoginName() + ":" + httpAuth.getPassword();
            byte[] enc = Base64.encodeBase64(toEncode.getBytes(), false);
            encoded = "Basic " + new String(enc);
        }
        return encoded;
    }

    /**
     * Extracts login data from the request using either the defined params (_login, _password),
     * or the basic auth header
     * @param req the current request
     * @return the auth data OR null if there is none to be found
     */
    public static HttpAuth extractRequestAuth(HttpServletRequest req) {
        return HttpRESTUtils.extractRequestAuth(req, null, null);
    }
     ...
     * or the basic auth header
     * @param req the current request
     * @param loginKey [OPTIONAL] the key name to look for in the params
     * @param passwdKey [OPTIONAL] the key name to look for in the params 
     * @return the auth data OR null if there is none to be found
     */
    public static HttpAuth extractRequestAuth(HttpServletRequest req, String loginKey, String passwdKey) {
        if (loginKey == null || "".equals(loginKey)) {
            loginKey = LOGIN;
        }
        if (passwdKey == null || "".equals(passwdKey)) {
            passwdKey = PASSWORD;
        }
        HttpAuth httpAuth = null;
        String login = req.getParameter(loginKey);
        if (login != null && login.length() > 1) {
            String password = req.getParameter(passwdKey);
            httpAuth = new HttpAuth(AuthType.PARAMS, login, password);
        }
        if (httpAuth == null) {
            ...
            if (authHeader != null && authHeader.startsWith("Basic ")) {
                try {
                    String basicEncode = authHeader.substring(6);
                    // decode the encoded username and password from base64
                    byte[] decoded = Base64.decodeBase64(basicEncode.getBytes());
                    String decodedStr = new String(decoded);
                    int pos = decodedStr.indexOf(':');
                    String username = decodedStr.substring(0, pos);
                    String password = decodedStr.substring(pos+1);
                    httpAuth = new HttpAuth(AuthType.BASIC, username, password);
                } catch (Exception e) {
                    httpAuth = null;
                }
            }
    ...
    /**
     * Extract the body of a request as a string
     * @param req the http request
     * @return the body as a string if there is one OR null if no body
     */
     ...
     *            the url path
     * @param position
     *            the position number in the path segments, 0 is always the prefix
     * @return the value at the given path position OR null if there is nothing at that position
     */
    public static String getPathSegment(String path, int position) {
        String segment = null;
    ...
    /**
     * Find the extension from a string and return the string without the extension and the
     * extension, an extension is a period (".") followed by any number of non-periods, the original
     * input is returned as the 0th item in the array <br/>
     ...
     * @param input
     *            any string
     * @return an array with the string without the extension or the original if it has none in
     *         position 1 and the extension in the position 2 (or null if no extension), position 0
     *         holds the original input string
     */
    public static String[] findExtension(String input) {
        ...
        if (input != null) {
            ...
            if (extensionLoc == 0) {
                ...
                if (extensionLoc > 0 && sepLoc < extensionLoc) {
